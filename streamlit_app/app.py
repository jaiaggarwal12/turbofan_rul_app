...
# Hero animation
# -----------------------------
engine_html = """ (UNCHANGED SVG HTML OMITTED FOR BREVITY) """
...
if page == "Project Details":
    ...
    st.write("CNN + BiLSTM + Attention on NASA C-MAPSS FD001 dataset")
...
if page == "Samples / Download":
    ...
    sample_code = """cycle,op_setting_1,...,s21"""
    st.code(sample_code)
    ...
    if df_test is None:
        st.error("FD001 test file not found at data/raw/test_FD001.txt")
    else:
        ...
        if not engine_df.empty:
            csv_data = engine_df.to_csv(index=False).encode("utf-8")
            st.download_button(
